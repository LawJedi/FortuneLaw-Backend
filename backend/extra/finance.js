	...
	*/
	...
	lib.calculateMonths = function(finAmount, finInterest, finPayment){
			...
			if(finInterest == 0){
				...
				r = finInterest/1200;
				result = Math.ceil((Math.log10(finPayment/(finAmount*r)) - Math.log10((finPayment/(finAmount*r))-1))/(Math.log10(1+r)));
			}
	...
	lib.calculateAmortization = function(finAmount, payment, finInterest, finDate){
			// correct months formula 
			// rate = annual rate
			function months(balance, rate, payment){
				r = rate/1200;
				return Math.ceil((Math.log10(payment/(balance*r)) - Math.log10((payment/(balance*r))-1))/(Math.log10(1+r)));
			}
			var finMonths = months(finAmount, finInterest, payment),
				balance = finAmount,
				totalInterest = 0.0,
				schedule = [],
				currInterest = null,
			...
			for(var i=0; i<finMonths; i++){
				...
				currInterest = Math.round((balance * finInterest/1200)*100)/100;
				if(balance+currInterest <= payment) {
					payment = balance+currInterest
				}
